...
function response(setup) {
    ...
    response.actions = {};
    var url = '';
    var hostname = settings.value.hostname || settings.options.response.hostname;
    ...
    function next(number) {
        if (settings.options.response.actions.next) {
            url = '';
            queryBuilder('skip');
            var dataSkip = parseInt(settings.value.query.limit) + parseInt(settings.value.query.skip);
    ...
    function prev(number) {
        if (settings.options.response.actions.prev) {
            url = '';
            queryBuilder('skip');
            var dataSkip = parseInt(settings.value.query.skip) - parseInt(settings.value.query.limit);
            if (dataSkip > 0) {
                urlBuilder('skip=' + dataSkip)
