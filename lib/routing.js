...
function routing (setup) {
  ...
  _.forEach(settings.models, function (n) {
    ...
    apps[n].put('/:' + n + 'Id', function (req, res, next) {
      ...
      data.saveAsync().then(function (thenData) {
        settings.response(res, {
          method: 'json',
          query: req.queryParameters,
          hostname: req.get('host') + req.path,
    ...
    apps[n].delete('/:' + n + 'Id', function (req, res, next) {
      var data = req[apps[n]]
      data.removeAsync().then(function (thenData) {
        settings.response(res, {
          method: 'json',
          query: req.queryParameters,
          hostname: req.get('host') + req.path,
    ...
    apps[n].get('/task/aggregate/', function (req, res, next) {
      if (req.queryParameters.aggregate) {
        settings.mongoose.model([n]).aggregateAsync(req.queryParameters.aggregate).then(function (results) {
          settings.response(res, {
    ...
    apps[n].get('/task/count/', function (req, res, next) {
      settings.mongoose.model([n]).countAsync(req.queryParameters.filter).then(function (totalCount) {
        settings.response(res, {
          count: totalCount,
          method: 'json',
          query: req.queryParameters,
