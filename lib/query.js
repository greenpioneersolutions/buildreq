...
var check = function (settings, req, callback) {
  ...
  auto({
    ...
    or: function (cb) {
      cb(null, andOrNorParameter(query.or, '$or') ? query.or : settings.defaults.or)
    },
    nor: function (cb) {
      cb(null, andOrNorParameter(query.nor, '$nor') ? query.nor : settings.defaults.nor)
    },
    and: function (cb) {
      cb(null, andOrNorParameter(query.and, '$and') ? query.and : settings.defaults.and)
    },
    ...
    if (data) {
      ...
      _.forEach(data, function (o, key) {
        _.forEach(o, function (d) {
          var obj = {}
          obj[key] = autoParse(d)
          queryParameters.filter[type].push(obj)
        })
      })
      return true
    } else {
  ...
  function whereParameter (check, type) {
    if (!_.isUndefined(check)) {
      debug('whereParameters:', check, ' ', type)
      whereBoolean = true
      whereParameters[type] = check
      return true
    } else {
      debug('whereParameters - false:', check, ' ', type)
      return false
    }
  }
        ...
        if (key === '$equals') {
          ...
          try {
            if (settings.modelNameCheck(_.replace(req.path, settings.options.routing.url, ''))) {
              output[where] = new RegExp(_.escapeRegExp(autoParse(n)), 'i') // valueParser(settings.schemas[settings.modelNameCheck(_.replace(req.path, settings.options.routing.url, ''))].paths[where].instance, n)), 'i')
            } else {
          ...
          try {
            if (_.isArray(n)) {
              if (settings.modelNameCheck(_.replace(req.path, settings.options.routing.url, ''))) {
                // FOREACH OVER THE ARRAY AND PARSE ALL VALUES
                output[where][key] = autoParse(n) // autoParse(n) // valueParser(settings.schemas[settings.modelNameCheck(_.replace(req.path, settings.options.routing.url, ''))].paths[where].instance, n)
              } else {
                if (settings.options.query.strict) {
                  if (settings.schema.indexOf(where) !== -1) {
              ...
              if (settings.modelNameCheck(_.replace(req.path, settings.options.routing.url, ''))) {
                output[where][key] = [autoParse(n)] // [valueParser(settings.schemas[settings.modelNameCheck(_.replace(req.path, settings.options.routing.url, ''))].paths[where].instance, n)]
              } else {
                if (settings.options.query.strict) {
                  if (settings.schema.indexOf(where) !== -1) {
                    output[where][key] = [autoParse(n)]
...
function queryMiddlewareFunc (setup) {
  return function queryMiddleware (req, res, next) {
    if (_.isEmpty(req.query)) {
      debug('Req.query is empty:', req.query)
      req.queryParameters = setup().defaults
      delete req.queryParameters.errorMessage
      delete req.queryParameters.warningMessage
      delete req.queryParameters.delete
      delete req.queryParameters.mongoose
      delete req.queryParameters.schema
      delete req.queryParameters.strict
      return next()
    }
...
function query (setup) {
  var settings = setup()
  if (_.isEmpty(settings.req.query)) {
    debug('Req.query is empty:', settings.req.query)
    var queryParameters = settings.defaults
    delete queryParameters.errorMessage
    delete queryParameters.warningMessage
    delete queryParameters.delete
    delete queryParameters.mongoose
    delete queryParameters.schema
    delete queryParameters.strict
