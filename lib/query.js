...
var check = function (settings, req) {
  ...
  function modelNameCheck (name) {
    debug('modelNameCheck name:', name, ' indexOf: ', models.indexOf(name))
    return models[models.indexOf(name)]
  }
  debug('defaults.mongoose:', defaults.mongoose)
  debug('query', query)
  debug('settings.options.query.mongoose', settings.options.query.mongoose)
      ...
      _.forEach(whereParameters, function (n, key) {
        if (key === '$equals') {
          try {
            ...
            warningHandler(where + '- The Where selector appears tobe the problem', 'Where-Path', err)
          }
        } else if (key === 'find') {
