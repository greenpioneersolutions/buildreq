...
var check = function (settings, req, callback) {
  ...
  function populateIdCheck (data, value) {
    ...
    if (!_.isArray(settings.defaults.limitToPopulateId)) {
      limitToPopulateItems = _.split(_.replace(settings.defaults.limitToPopulateId, ',', ' '), ' ')
    }
    ...
    _.forEach(data, function (n, key) {
      if (models.indexOf(n) !== -1 || schema.indexOf(n) !== -1) {
        ...
        if (n)warningHandler(n, 'populateId - Not in settings.schemas provided', settings.defaults.populateId)
      }
    })
  ...
  function populateItemsCheck (data, value) {
    ...
    _.forEach(data, function (n, key) {
      if (schema.indexOf(n) !== -1) {
        ...
        if (n)warningHandler(n, 'populateItems', settings.defaults.populateItems)
      }
    })
