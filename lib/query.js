  ...
  function whereCheck (where) {
    ...
    if (whereBoolean) {
      ...
      _.forEach(whereParameters, function (n, key) {
        if (key === '$equals') {
          output[where] = valueParser(schemas[_.replace(req.path, settings.options.routing.url, '')].paths[where].instance, n)
        } else if (key === 'find') {
          output[where] = new RegExp(valueParser(schemas[_.replace(req.path, settings.options.routing.url, '')].paths[where].instance, n), 'i')
        } else if (key === '$nin' || key === '$in' || key === '$all') {
          if (_.isArray(n)) {
            output[where][key] = valueParser(schemas[_.replace(req.path, settings.options.routing.url, '')].paths[where].instance, n)
          } else {
            output[where][key] = [valueParser(schemas[_.replace(req.path, settings.options.routing.url, '')].paths[where].instance, n)]
          }
        } else {
